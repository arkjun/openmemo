...
if (specifiedTarget) {
  ...
  buildTargets = allTargets.filter(t => requestedTargets.some(rt => t.target.includes(rt) || t.pkg.includes(rt)));
} else if (currentOnly) {
...
for (const { target, pkg, ext } of buildTargets) {
  ...
  try {
    ...
    if (error instanceof Error) {
      // biome-ignore lint/suspicious/noExplicitAny: Bun shell error type lacks proper stderr typing
